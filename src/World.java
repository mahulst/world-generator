...
public class World implements Runnable {
	...
	public void mapWorld() {
		...
		//Precipitation
		//By far the slowest process
		ArrayList<Point> current=new ArrayList<Point>(),future=new ArrayList<Point>();
		for (int y=0;y<HEIGHT;y+=1)
			for (int x=0;x<WIDTH;x+=1)
		...
		for (int precip=100;precip>0;precip--) {
			for (int i=0;i<current.size();i++) {
				if (current.get(i).precipitation==precip) {
					for (Point p2:current.get(i).adjacent) {
						if (p2.precipitation==0) {
							p2.precipitation=current.get(i).precipitation-(p2.z-WATERLEVEL)/HUMIDITY-1;
							current.add(p2);
							future.remove(p2);
						}
					}
	...
	private double elevationNoise(double x,double y) {
